...
import { Mail, Send, CheckCircle2, ArrowRight, Copy } from 'lucide-react';
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="lg:col-span-6 space-y-8">
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl sm:text-3xl font-semibold font-display text-white">Connect directly with Prajith P.</h3>
            ...
            <div className="space-y-4 pt-2 font-sans">
              <div className="p-5 rounded-2xl bg-[#121217] border border-white/10 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 truncate">
                  ...
                  <a href={`mailto:${PERSONAL_INFO.email}`} className="text-[#0071E3] hover:text-[#0077ED] font-semibold text-base sm:text-lg flex items-center gap-1 transition-colors truncate">
                    <span>Email Prajith</span><ArrowRight className="w-4 h-4" />
                  </a>
                </div>
                <button onClick={handleCopyEmail} className="p-2.5 rounded-full bg-white/5 hover:bg-white/15 text-slate-300 transition-colors cursor-pointer shrink-0 border border-white/10" title="Copy email address">
                  {copiedEmail ? <CheckCircle2 className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                </button>
              </div>

              <div className="grid grid-cols-2 gap-4 pt-2">
                <a href={PERSONAL_INFO.linkedin} target="_blank" rel="noopener noreferrer" className="p-5 rounded-2xl bg-[#121217] border border-white/10 hover:border-[#0071E3] flex items-center justify-between text-[#0071E3] font-semibold text-base transition-all group">
                  <div className="flex items-center gap-2"><LinkedinIcon className="w-4 h-4 text-[#0071E3]" /><span>LinkedIn</span></div>
                  <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </a>
                <a href={PERSONAL_INFO.github} target="_blank" rel="noopener noreferrer" className="p-5 rounded-2xl bg-[#121217] border border-white/10 hover:border-[#0071E3] flex items-center justify-between text-[#0071E3] font-semibold text-base transition-all group">
                  <div className="flex items-center gap-2"><GithubIcon className="w-4 h-4 text-[#0071E3]" /><span>GitHub</span></div>
                  <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </a>
              </div>
            </div>
            ...
            <div className="p-8 rounded-3xl bg-[#121217] border border-white/10">
              ...
              {formSubmitted ? (
                <div className="p-8 text-center space-y-3 rounded-2xl bg-white/5 border border-white/10">
                  ...
                  <p className="text-xs text-[#A1A1A6] font-sans">Thank you. Prajith will review your inquiry and get back to you promptly.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4 font-sans">
                  ...
                  <button type="submit" className="w-full apple-btn-blue py-3.5 text-sm font-medium flex items-center justify-center gap-2 cursor-pointer shadow-md"><Send className="w-4 h-4" /><span>Send Message</span></button>
                </form>
              )}
            </div>
          </motion.div>
